...
public class RewardNetworkImpl implements RewardNetwork {
	...
	public RewardConfirmation rewardAccountFor(Dining dining) {
		...
		Account client = accountRepository.findByCreditCard(dining.getCreditCardNumber());
		Restaurant restaurant = restaurantRepository.findByMerchantNumber(dining.getMerchantNumber());
		MonetaryAmount reward = restaurant.calculateBenefitFor(client, dining);
		AccountContribution contribution = client.makeContribution(reward);
		accountRepository.updateBeneficiaries(client);
		RewardConfirmation number = rewardRepository.confirmReward(contribution, dining);
